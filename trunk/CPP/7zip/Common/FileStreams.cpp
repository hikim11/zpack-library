...
CIOStream::Buffers CIOStream::buffers_;
size_t CIOStream::totalSize_ = 0;
...
Byte * CIOStream::Read(size_t & size)
{
	Enumerate();
...
STDMETHODIMP CIOStream::Write(const void *data, UInt32 size, UInt32 *processedSize)
{
	...
	buffers_.push_back( Buffer( buf, size ) );
...
void CIOStream::Enumerate()
{
	if( buffers_.size() > 1 )
	{
		...
		for( Buffers::iterator i=buffers_.begin(); i!=buffers_.end(); ++i )
		{
			...
			copiedSize += buffer.size;
		}

		Clear();

		totalSize_ = copiedSize;

		buf[totalSize_] = 0;

		buffers_.push_back( Buffer( buf, copiedSize ) );
	}
}
...
void CIOStream::Clear()
{
	totalSize_ = 0;

	/*
	parallel::_for( 0, (int)buffers_.size(),
		[](int i, bool & breakFlag)
		{
			if( breakFlag )
				return;

			delete[] buffers_[i].buf;
		}		
		, 4
	);
	*/
...
{
	if( p )
		delete[] p;//umtl::memory_manager::get().free( p );
}
