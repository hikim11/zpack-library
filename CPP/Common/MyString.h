...
bool MyStringParallelCompareNoCase( const wchar_t *st, const wchar_t *s2, size_t len );
...
public:
  ...
  bool ParallelCompareNoCase( const T * s ) const
	{ return MyStringCompareNoCase( _chars, s ) == 0; }

  bool ParallelCompareNoCase( const CStringBase& s ) const
	{ return _length == s._length ? MyStringParallelCompareNoCase( _chars, s._chars, _length ) : false; }

  /*
  bool operator==( CStringBase const & other )
  {
	  return CompareNoCase( other );
  }
...
bool operator==(const CStringBase<T>& s1, const CStringBase<T>& s2)
  { return (s1.ParallelCompareNoCase(s2)); }
...
bool operator==(const T *s1, const CStringBase<T>& s2)
  { return (s2.ParallelCompareNoCase(s1)); }
