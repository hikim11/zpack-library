  ...
  const UStringVector &commandStrings)
{
  ...
  for (int i = 0; i < numCommandStrings; i++)
  {
    ...
    if (stopSwitch)
      ...
      if (s.Compare(kStopSwitchParsing)==0)
        stopSwitch = true;
      else
...
bool CParser::ParseString(const UString &s, const CSwitchForm *switchForms)
{
  ...
  while (pos < len)
  {
    ...
    for (int switchIndex = 0; switchIndex < _numSwitches; switchIndex++)
    {
	  ...
	  if (_wcsnicmp(switchForms[switchIndex].IDString
		  , s.Length() > pos ? s+pos : s + s.Length() - 1
		  , s.Length() < switchLen ? s.Length() : switchLen) == 0)
	  //if( MyStringCompare( s.Length() < pos ? s+pos : s + s.Length() - 1,switchForms[switchIndex].IDString ) == 0 )
      //if (temp.Compare(switchForms[switchIndex].IDString) == 0)
