    ...
    UInt64 &numErrors, ZDB * zdb)
{
  ...
  for (int i = 0; i < numArcs; i++)
  {
    ...
    HRESULT result = archiveLink.Open2(codecs, formatIndices, stdInMode, NULL, archiveName, &openCallback, zdb);
    if (result != S_OK)
    {
      if (result == E_ABORT)
        return result;
    ...
    for (UInt32 i = 0; i < numItems; i++)
    {
	  ...
	  // 파일 속성 출력 (생성 날짜, 만든날짜, 이름, 읽기속성, 이름 등등)
