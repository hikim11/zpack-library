...
{
	/*
	zmodifyer zm( L"7zip.7z" );

	UStringVector delFiles;

	delFiles.Add( L"a.txt" );
...
/*
  ...
  CMyComPtr<
    ...
    IUnknown
    #endif
    > compressCodecsInfo = codecs; // compressCodecsInfo 는 벤치마크를 하는경우 사용함.
  ...
  if (options.Command.CommandType == NCommandType::kInfo) // 압축 코덱 정보 출력
  {
  ...
  else if (isExtractGroupCommand || options.Command.CommandType == NCommandType::kList)
  {
    ...
    if (isExtractGroupCommand)
    {
      ...
      HRESULT result = DecompressArchives(
          ...
          eo, &openCallback, ecs, errorMessage, stat, 0);
  ...
  else if (options.Command.IsFromUpdateGroup())
  {
    ...
    HRESULT result = UpdateArchive(codecs,
        ...
        errorInfo, &openCallback, &callback, 0);
