	...
	ZDB * zdb)
{
  ...
  for (int i = 0; i < orderIndices.Size(); i++)
  {
    ...
    HRESULT result;
    if (stream)
      result = archive->Open(stream, &kMaxCheckStartPosition, callback, zdb);
    else
    {
...
HRESULT CArc::OpenStreamOrFile(
    ...
    bool stdInMode,
    IInStream *stream,
    IArchiveOpenCallback *callback, ZDB* zdb)
{
  ...
  */
  ...
  return OpenStream(codecs, formatIndex, stream, seqStream, callback, zdb);
}
    ...
    IArchiveOpenCallback *callback, ZDB * zdb)
{
  ...
  for (;;)
  {
    ...
    if (Arcs.IsEmpty())
    {
      ...
      RINOK(arc.OpenStreamOrFile(codecs, formatIndex, stdInMode, stream, callback, zdb));
      Arcs.Add(arc);
      continue;
    }
    ...
    HRESULT result = arc2.OpenStream(codecs, formatIndex, subStream, NULL, callback, zdb);
...
HRESULT CArchiveLink::Open2(CCodecs *codecs,
    ...
    IInStream *stream,
    const UString &filePath,
    IOpenCallbackUI *callbackUI, ZDB * zdb)
{
  VolumesSize = 0;
  COpenCallbackImp *openCallbackSpec = new COpenCallbackImp;
  ...
  RINOK(Open(codecs, formatIndices, stdInMode, stream, filePath, callback, zdb));
    ...
    IArchiveOpenCallback *callback, ZDB* zdb)
{
  ...
  if (Arcs.Size() == 0)
    return Open2(codecs, CIntVector(), false, NULL, filePath, 0, zdb);
  ...
  if (!fileStreamSpec->Open(filePath))
    return GetLastError();
  HRESULT res = GetArchive()->Open(stream, &kMaxCheckStartPosition, callback, zdb);
  IsOpen = (res == S_OK);
  return res;
}
