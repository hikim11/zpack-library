...
static const wchar_t *kDefaultArchiveType = L"zp";
...
HRESULT UpdateArchive(
    ...
    IUpdateCallbackUI2 *callback, ZDB* zdb)
{
  ...
  if (archiveFileInfo.Find(archiveName))
  {
	...
	HRESULT result = archiveLink.Open2(codecs, formatIndices, false, NULL, archiveName, openCallback, zdb);

	// 오픈이 실패했다면 손상된 파일 이므로
	if( result == S_FALSE )
	{
		// 파일을 지우고 계속 작업
		SetFileAttributes(archiveName, FILE_ATTRIBUTE_NORMAL);
