...
//{
	...
	inline std::wstring lexical_cast( char const * t )
	{
		...
		if( t )
		{
			...
			MultiByteToWideChar( CP_ACP, 0, t, len, buf, len );
			buf[len-1] = 0;
	...
	inline std::string lexical_cast( wchar_t const * t )
	{
		...
		if( t )
		{
			...
			len = WideCharToMultiByte( CP_ACP, 0, t, -1, buf, len, 0, 0 );
			buf[len-1] = 0;
	...
	public:
		...
		std::string operator() (const std::wstring& src) const
		{
			...
			/*
			parallel::_for( 0, buf.size(),
				[&buf](int i, bool & breakFlag)
			{
				if( buf[i] == ' ' || buf[i] == '#' )
					buf[i] = '+';
			}
	...
	}; // http://blog.naver.com/laster40?Redirect=Log&logNo=50623019
